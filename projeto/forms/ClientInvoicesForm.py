...
class ClientInvoicesForm(forms.Form):
    ...
    sales_order_1 = forms.IntegerField(required=True, error_messages={'required': 'Necessário selecionar pelo menos uma guia de remessa'})
    ...
    def clean(self):
        ...
        i = 1
        field_name = 'sales_order_%s' % (i,)

        while self.data.get(field_name):
            sales_order = self.data[field_name]
            if sales_order in sales_orders:
                self.add_error(field_name, 'Duplicate')
            else:
        ...
        self.cleaned_data['sales_orders'] = sales_orders

    def get_material_receipts_fields(self):
        for field_name in self.fields:
            if field_name.startswith('sales_order_'):
